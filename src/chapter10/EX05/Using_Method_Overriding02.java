...
public class Using_Method_Overriding02 {
	...
	public static void main(String[] args) {
		...
		// 2. Animal 타입으로 선언 + 자식 클래스로 객체 생성
			// 자신의 객체를 생성할 때 부모 타입으로 정의 후 부모의 메소드를 호출 시 오버라이딩 됨 메소드 출력
		...
		bb1.cry();		// bb1.cry() : Animal 의 cry()를 호출 --> 오버라이딩
		cc1.cry();		// cc1.cry() : Animal 의 cry()를 호출 --> 오버라이딩
		dd1.cry();		// dd1.cry() : Animal 의 cry()를 호출 --> 오버라이딩
		
		// Animal 타입으로 선언이 되어 있으므로 배열이나 컬렉션에 넣어서 아주 쉽게 관리
		// 배열의 특징 : 1. 동일한 타입을 저장 2. 방의 크기가 선언되면 바꿀 수 없음
		...
		for (int i=0 ; i<arr3.length ; i++) {
			arr3[i].cry();								// Animal 의 cry() 를 호출 시 오버라이딩된 메소드가 출력됨
		}
